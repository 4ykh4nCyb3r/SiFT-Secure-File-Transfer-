...
class SiFT_LOGIN:
    ...
    #def derive_transfer_key(self, client_random, server_random, request_hash):
        #key_material = bytes.fromhex(client_random) + bytes.fromhex(server_random)
        #return HKDF(key_material, 32, request_hash, SHA256)
...
'''
        self.mtp.transfer_key = self.derive_transfer_key(
            login_req_struct['client_random'], login_res_struct['server_random'].hex(), request_hash
        )
        self.mtp.temp_key = None  # Discard temporary key
        '''