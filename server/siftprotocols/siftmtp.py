...
class SiFT_MTP:
    ...
    def encrypt_payload(self, msg_type, header, payload, sqn, rnd):
        #key = generate_tr_key(self.peer_ecc_public_key) if msg_type == self.type_login_req or msg_type==self.type_login_res else self.transfer_key #added or self.type_login_res to the condition
        ...
        if self.DEBUG:
            ...
            #print("Header in encrypt_payload: ", header.hex())
            #print(payload)
    ...
    def decrypt_payload(self, msg_type, header, ciphertext, tag, sqn, rnd):
        ...
        '''
        if self.DEBUG:
            print(f'Nonce in decrypt_payload: {nonce.hex()}')
            print(f'key in decrypt_payload: {key.hex()}')
            print(f'ciphertext in decrypt_payload: {ciphertext.hex()}')
            print(f'Tag in decrypt_payload: {tag.hex()}')
            print()
        #-----------
        '''
        cipher = AES.new(key, AES.MODE_GCM, nonce=nonce, mac_len=12)
        #----
        #print("Header in decrypt_payload: ", header.hex())
        cipher.update(header)
        # --- mine ----
        try:
    ...
    def receive_msg(self):
        ...
        ''' ##Debugging
        print("")
        print("Server ciphertext: ", ciphertext.hex())
        print("Server tag: ", tag.hex())
        print("parsed_msg_hdr['typ']: ", parsed_msg_hdr['typ'].hex())
        print("parsed_msg_hdr['sqn']: ", parsed_msg_hdr['sqn'].hex())
        print("parsed_msg_hdr['rnd']: ", parsed_msg_hdr['rnd'].hex())
        print("")
        '''
    ...
    def send_msg(self, msg_type, msg_payload):
        ...
        if self.DEBUG:
            ...
            print(f'Random number: {rnd.hex()}')
            print(f'Sequence number: {self.send_sqn}')
            '''
        # ----------------
